...
def dir_add_column(target_dir, add_dir):
    target_file_list = glob.glob(target_dir + '*.conllu')
    add_file_list = glob.glob(add_dir + '*.conllu')
...
def add_column(target_file, add_file, column_name):
    """
    ...
    for each_line in target_file:
        ...
        if string_buffer[0] == '#':
            ...
            print(string_buffer)
            continue
        print(string_buffer + '\t' + str(columndata[sent_id][idx]))
        idx += 1
...
def split_to_conllu_corpus(input_file, output_dir_path, num_of_sent=1000):
    ...
    def open_output_file(_output_dir_path, _output_file_name, _file_index):
        return open(_output_dir_path + '\\' + _output_file_name + '_' + str(_file_index) +
